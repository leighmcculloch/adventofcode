...
func createMatrix(input string) matrix {
	m := matrix{}
	x, y := mid, mid
...
func printMatrix(matrices ...matrix) {
	for y := 0; y < size; y++ {
		for x := 0; x < size; x++ {
			on := false
...
func main() {
	//m1 := createMatrix("R8,U5,L5,D3")
	//m2 := createMatrix("U7,R6,D4,L4")
	//m1 := createMatrix("R75,D30,R83,U83,L12,D49,R71,U7,L72")
	//m2 := createMatrix("U62,R66,U55,R34,D71,R55,D58,R83")
	...
	for _, p := range points {
		distance := abs(mid-p.x) + abs(mid-p.y)
