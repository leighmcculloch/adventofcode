...
pub fn main() !void {
    ...
    // Calcs.
    const totalSpace: usize = 70000000;
    const usedSpace = root.totalSize();
    const freeSpace = totalSpace - usedSpace;
    const needFreeSpace = 30000000;
    var ctx: Ctx = .{
        .needToFree = needFreeSpace - freeSpace,
    };

    // List all directories greater than 100000 in size.
    root.visit(&ctx, findBestToFree);
    print("{any}\n", .{ctx.bestToFree});
}

const Ctx = struct {
    needToFree: usize,
    bestToFree: ?usize = null,
};

fn findBestToFree(ctx: *Ctx, n: *Node) void {
    if (n.typ == .dir) {
        const s = n.totalSize();
        if (s >= ctx.needToFree) {
            if (ctx.bestToFree) |currBestToFree| {
                if (s <= currBestToFree) {
                    ctx.bestToFree = s;
                }
            } else {
